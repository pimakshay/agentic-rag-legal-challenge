...
- `LegalQuestionRouter` detects case IDs, neutral citations, article references, conservative law title candidates / law numbers, and page-specific cues before retrieval.
...
- `doc_type`: `case` or `law`
- `structure_available`
- `blocks`: ordered ingest blocks with `block_index`, `page_number`, `text`, `type`, `level`
- law docs additionally persist `alias_keys`, `article_index`, and `section_heading_index`
...
The router is question-local and optimized for the challenge corpus:

- case IDs: `CA 005/2025`, `CFI 057/2025`, `SCT 295/2025`
- article references: `Article 28(1)`
- law identifiers: conservative title candidates and explicit `DIFC Law No. X of YYYY`
- page hints: `title page`, `cover page`, `page 2`, `last page`
- comparison/common-entity/common-judge signals

Routing effects:

- exact case IDs prefilter by `claim_number`
- exact neutral citations prefilter by `neutral_citation`
- exact law numbers prefilter by `official_citation`
- exact law title candidates prefilter by ingestion-time `alias_keys`
- law + article queries prefer ingestion-time `article_index` spans inside already resolved law docs
